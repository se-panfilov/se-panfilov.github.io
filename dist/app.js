...
;
angular.module("sp-io.templates", []).run(["$templateCache", function($templateCache) {$templateCache.put("landing/landing.html","<main-header></main-header><div class=\"page_content about_page\"><div class=container><h3>Hi there. It\'s still under construction yet.</h3></div><section flex=flex class=skills><md-grid-list md-cols-sm=1 md-cols-md=2 md-cols-gt-md=6 md-row-height-gt-md=1:1 md-row-height=4:3 md-gutter=8px md-gutter-gt-sm=4px><md-grid-tile ng-repeat=\"tile in tiles\" md-rowspan={{tile.span.row}} md-colspan={{tile.span.col}} md-colspan-sm=1 ng-style=\"{\'background\': tile.color}\" ng-class=tile.background><md-icon md-svg-icon={{tile.icon}}></md-icon><md-grid-tile-footer><h3>{{tile.title}}</h3></md-grid-tile-footer></md-grid-tile></md-grid-list></section></div>");
